...
class TestNativeSampler:
    ...
    def test_auto_strategy_imbalanced(self):
    ...
    def test_auto_strategy_balanced(self):
        """Test auto strategy on balanced data"""
        X, y = make_classification(
            n_samples=100, n_features=4, weights=[0.5, 0.5], random_state=42  # Balanced
        )
        X_df = pd.DataFrame(X, columns=[f"f_{i}" for i in range(X.shape[1])])
        y_series = pd.Series(y)
...
class TestApplySamplingFunction:
    ...
    def test_apply_sampling_smote(self):
    ...
    def test_apply_sampling_undersample(self):
    ...
    def test_apply_sampling_auto(self):
...
class TestSamplingEdgeCases:
    ...
    def test_binary_classification_edge_case(self):
    ...
    def test_multiclass_sampling(self):
    ...
    def test_stratified_sampler_reproducibility(self):
