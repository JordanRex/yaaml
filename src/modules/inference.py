...
class yaamlInference():

    def __init__(self, model, interpreter_algo):
    ...
    def lime_interpreter(feat_names, classnames, categindices, categnames,
        ...
        exp = explainer.explain_instance(
            xtest[n], model.predict_proba, num_features=num_feature)
    ...
    def shap_interpreter(model, train, test, n, method='tree'):
        ...
        if method == 'tree':
            ...
            shap_explainer = shap.KernelExplainer(
                model.predict_proba, shap.kmeans(train[:100], 5))
            shap_values = shap_explainer.shap_values(test.values)
...
x = yaamlInference(interpreter_algo='lime', model=model, train=X_train, test=X_test, feat_names=feat_names,
